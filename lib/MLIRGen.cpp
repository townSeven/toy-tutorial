...
public:
  ...
  mlir::ModuleOp mlirGen(ModuleAST &moduleAST) {
    ...
    for (FunctionAST &f : moduleAST)    // @town: traverse AST
      mlirGen(f);
...
private:
  ...
  mlir::toy::FuncOp mlirGen(FunctionAST &funcAST) {
    ...
    mlir::toy::FuncOp function = mlirGen(*funcAST.getProto());    // @town: traverse 'Proto&Params'
    /* @town：
        at this point we have codegen'd：
    ...
    */
    ...
    if (mlir::failed(mlirGen(*funcAST.getBody()))) {    // @town: traverse 'Block'
      function.erase();
      return nullptr;
    }
    ...
    // [ch4] If this function isn't main, then set the visibility to private.
  ...
  mlir::Value mlirGen(BinaryExprAST &binop) {
    ...
    //    and propagate.
    //
    mlir::Value lhs = mlirGen(*binop.getLHS());   // @town: goto 'Call'
    if (!lhs)
      return nullptr;
  ...
  mlir::LogicalResult mlirGen(ReturnExprAST &ret) {
    ...
    if (ret.getExpr().has_value()) {
      if (!(expr = mlirGen(**ret.getExpr())))     // @town：goto 'BinOp'
        return mlir::failure();
    }
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    for (auto &expr : call.getArgs()) {
      auto arg = mlirGen(*expr);    // @town: goto ''
      if (!arg)
        return nullptr;
    ...
    if (callee == "transpose") {
      ...
      return builder.create<TransposeOp>(location, operands[0]);  // @town: create TransposeOp
    }
